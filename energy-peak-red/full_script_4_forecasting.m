...
for i = 1:M
    ...
    if ~(i == 5 || i == 6)
        ...
        allowed_mask = (t(1:Nint) >= 7) & (t(1:Nint) <= 21);
        AllowedWindows{i} = allowed_mask;
    end
end
...
% plot(t, L_fore_smart,     '-.k','LineWidth',1.5);
% plot(t, L_total_milp_true,'r',  'LineWidth',1.5);
plot(t, L_total_rand_true,'b',  'LineWidth',1.5);
xlabel('Time [h]');
ylabel('Load [kW]');
xlim([0,24])
xticks(0:3:24)
ylim([0,5])
legend('True base load', ...
       sprintf('Random schedule (peak=%.2f)',P_rand_true), ...
       'Location','NorthWest');
...
plot(t, L_true,           'k',  'LineWidth',2.1);
plot(t, L_fore_smart,     '-.k','LineWidth',1.5);
plot(t, L_total_milp_true,'r',  'LineWidth',1.6);
% plot(t, L_total_rand_true,':b',  'LineWidth',1.6);
xlabel('Time [h]');
ylabel('Load [kW]');
xlim([0,24])
...
legend('True base load', 'Base load forecast', ...
       ...
       'Location','NorthWest');
% title('Optimal Appliance Scheduling vs Random Feasible Scheduling');
% sgtitle('Random Feasible Scheduling vs Optimal Scheduling')
