...
function out = sim_case_extRLS(a,b,K,x0,w,Delta,Xmax,qx,ru)
    ...
    % feature sizes
    nx = 2;     % number of xq lags (xq_k, xq_{k-1}, xq_{k-2})
    nu = 2;     % number of u lags  (u_{k-1}, u_{k-2})
    ...
    for k=1:N
        ...
        x_pred  = a*xhat(k) + b*u(k);
        xq_pred = q_uniform(x_pred, Delta, Xmax);   % what the ADC would output
        e       = xq(k+1) - xq_pred;                % quantized innovation
